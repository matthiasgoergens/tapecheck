(* Tape shim over the PATCHED splittable_random (vendored as Sr_real,
   ...
   whole interception is one abstract [Intercept.t] observer per stream: every
   ...
   Streams (design/stream-keyed-tapes.md): each observer closes over
...
module For_tape = struct
  let rec hooks ?(record_spans = true) tape key : Sr_real.Intercept.t =
    Sr_real.Intercept.create
      ...
      ~on_span_start:
        (fun label ~deletable ~discardable ~descendable ~reorderable ->
          if record_spans
             && (deletable || discardable || descendable || reorderable)
          then begin
      ...
      ~on_span_stop:
        (fun ~deletable ~discardable ~descendable ~reorderable ~discarded () ->
          if record_spans
          then
            Tape.on_span_stop tape ~stream:key ~deletable ~discardable
              ~descendable ~reorderable ~discarded)
      ~on_split:
        (fun () ->
          Some
            (hooks ~record_spans tape (Tape.on_split tape ~stream:key)))
      ~on_perturb:
        (fun salt ->
          match Tape.on_perturb tape ~stream:key ~salt with
          | None -> None
          | Some salted -> Some (hooks ~record_spans tape salted))
      ()
