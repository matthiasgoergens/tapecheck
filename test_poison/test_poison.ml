(* Port of Hypothesis's tests/quality/test_poisoned_trees.py.
   ...
   The stock arm below retains the original flat PRNG-level measurement; the
   structural arm adds only recursive span brackets and exercises Tapecheck's
   opt-in descendant pass.
   ...
   Span boundaries are exactly what let the structural arm relocate the
   subtree intact: it now reaches 34/34 while the stock arm remains 12/34. *)
open Base
module G = Base_quickcheck.Generator

type Splittable_random.span_label += Poison_tree
...
let tree_gen ~spans ~p ~max_depth =
  ...
  let rec go depth =
    ...
    then body
      ...
      G.create (fun ~size ~random ->
        Splittable_random.with_span ~descendable:true random Poison_tree
          ~f:(fun () -> G.generate body ~size ~random))
  in
  go max_depth
...
let gen ~spans ~p ~max_depth =
  G.both (tree_gen ~spans ~p ~max_depth) (G.int_uniform_inclusive 0 marker_value)
...
let run_size ~spans ~size ~seed =
  ...
  | Some (img, t, p, max_depth) ->
    ...
    List.iteri starts ~f:(fun idx u ->
      ...
      let pt, pm = replay poisoned ~spans ~p ~max_depth in
      if has_poison pt && pm = marker_value then begin
        Int.incr total;
        let test (t, m) = not (has_poison t && m = marker_value) in
        match
          Tape_engine.resume (gen ~spans ~p ~max_depth) ~test poisoned ~budget:20_000
            ~max_shrinks:5000
        with
...
let measure ~name ~spans =
  Stdio.printf "%s:\n" name;
  let results =
    List.concat_map [ 2; 5; 10 ] ~f:(fun size ->
      List.filter_map [ 0; 1 ] ~f:(fun seed -> run_size ~spans ~size ~seed))
  in
...
let () =
  let ok, total =
    measure
      ~name:"poisoned trees, stock generator (Hypothesis tests/quality/test_poisoned_trees.py)"
      ~spans:false
  in
  ...
  in
  Test_support.report "pass_to_descendant preserves every poisoned leaf"
    (structural_ok = 34 && structural_total = 34)
    (Printf.sprintf "%d/%d (expected 34/34)" structural_ok structural_total);
