...
Scenario 1: queue bisimulation (fair case) -- 300 trials, 200 cases/trial
  tapecheck  found 300/300, exact minimal 300/300, avg ops 3.00, max 3, avg shrink cost 77.3
  qcheck-stm found 300/300, exact minimal 300/300, avg ops 3.00, max 3, avg shrink cost 13.9

Scenario 2: handle allocator (adversarial) -- 300 trials, 200 cases/trial
  tapecheck  found 300/300, exact minimal 300/300, avg ops 2.00, max 2,  avg shrink cost 76.7
  qcheck-stm found 300/300, exact minimal 232/300, avg ops 2.35, max 12, avg shrink cost 12.4
```
...
## 3. Caveat the original summary omitted

tapecheck costs **~5.5x more shrink attempts on the scenario where it
has no advantage at all** (77.3 vs 13.9, both reaching the identical
minimal 3-op sequence). That is a real cost and belongs next to the
headline whenever it is quoted. The honest statement is:

> On properties whose operations do not reference earlier results, the
> tape buys nothing and costs about 5.5x the shrink attempts. Where
> later operations do reference earlier ones, it reaches the true
> minimum every time where qcheck-stm reaches it 77% of the time, and
> the extra attempts are the mechanism rather than an overhead — they
> are candidates qcheck-stm's shrinker cannot construct at all.
